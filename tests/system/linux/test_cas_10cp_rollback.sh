...
if grep -q '"ok":true' <<<"$RESP"; then
    pass "rollback cp5 accepted"
else
    fail "rollback cp5 resp=$RESP"
fi
...
[[ "$FAIL" -eq 0 ]] && echo "PASS test_cas_10cp_rollback" || { echo "FAIL test_cas_10cp_rollback"; exit 1; }
